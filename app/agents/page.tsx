...
export default function AgentsPage() {
  ...
  if (approvalReceipt.isError || registrationReceipt.isError) {
    return <><PageHero eyebrow="CeloHT · Agent Network" title="Agent registration failed" lead="The wallet transaction was submitted but the blockchain receipt did not confirm it. No agent registration is reported." /><Section><Card className="max-w-2xl"><CardHeader><CircleAlert size={18} aria-hidden="true" /><CardTitle>Transaction failed</CardTitle></CardHeader><CardDescription>{approvalReceipt.isError ? "USDm approval was rejected or reverted." : "Agent registration was rejected or reverted."}</CardDescription><Button className="mt-5" onClick={resetRegistration}>Try again</Button></Card></Section></>;
  }
