...
export default function HomePage() {
  ...
  return (
    <div className="app-shell dashboard-grid min-h-[calc(100vh-65px)] px-4 py-8 pb-28 sm:px-6 lg:px-8 lg:py-12 lg:pb-16">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]"><section className="rounded-2xl border border-white/10 bg-white/[0.06] p-6"><div className="flex items-center gap-3"><ShieldCheck className="text-cyan-300" size={20} aria-hidden="true" /><h2 className="font-display font-semibold text-white">Recent activity</h2></div><p className="mt-4 text-sm leading-6 text-parchment-100/65">{isConnected ? "No indexed activity yet. Confirmed transfers will appear here when a Celo indexer is configured." : "Connect your wallet to view activity. CeloHT never invents transaction history."}</p><Link href="/transactions" className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-gold-300">Open activity <ExternalLink size={14} aria-hidden="true" /></Link></section><section className="rounded-2xl border border-white/10 bg-white/[0.06] p-6"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-parchment-100/55">Learn and participate</p><div className="mt-5 grid grid-cols-2 gap-4"><MiniMetric icon={<BookOpen />} value={`${courses.length}`} label="Academy courses" /><MiniMetric icon={<Sprout />} value="—" label="Live forest data" /></div><p className="mt-5 text-xs text-parchment-100/40">Courses are available locally. Forest totals appear after verified contracts are connected.</p></section></div>
      </div>
    </div>
  );
}
