...
import { ArrowUpRight, BookOpen, Copy, ExternalLink, Network, Send, ShieldCheck, WalletCards, Users, Sprout } from "lucide-react";
import { useAccount, useBalance, useChainId } from "wagmi";
import { ConnectWalletButton } from "@/ConnectWalletButton";
import { getUsdmAddress } from "@/lib/contracts";
import { formatTokenAmount } from "@/lib/utils";
import { demoCourses, demoImpact, demoTransactions } from "@/lib/demo-data";
    ...
    <div className="app-shell dashboard-grid min-h-[calc(100vh-65px)] px-4 py-8 pb-28 sm:px-6 lg:px-8 lg:py-12 lg:pb-16">
      ...
      <div className="dashboard-reveal dashboard-reveal-delay grid gap-5 lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,0.55fr)]">
        <section className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-navy-800 to-[#123b38] p-6 text-white shadow-2xl shadow-black/20 sm:p-8" aria-labelledby="balance-heading">
          <div className="flex items-start justify-between gap-4"><div><p className="text-sm text-white/60">Portfolio balance <span className="ml-2 rounded-full bg-gold-300/15 px-2 py-1 text-[10px] text-gold-200">LIVE</span></p><h2 id="balance-heading" className="mt-3 font-display text-4xl font-semibold tracking-tight">{!isConnected ? "Connect wallet" : celoBalance.isLoading ? "Loading..." : celoBalance.error ? "Unavailable" : `${celoAmount} CELO`}</h2></div><div className="rounded-xl border border-white/10 bg-white/5 p-3"><WalletCards size={22} className="text-gold-300" aria-hidden="true" /></div></div>
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.06] p-6" aria-labelledby="network-heading"><div className="flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-parchment-100/55">Network center</p><StatusDot tone={isConnected && networkReady ? "good" : isConnected ? "warn" : "muted"} /></div><h2 id="network-heading" className="mt-4 font-display text-xl font-semibold text-white">Celo Network</h2><p className="mt-2 text-sm text-parchment-100/65">{!isConnected ? "Connect a wallet to check your network." : networkReady ? `${chain?.name ?? "Celo"} connected` : "Please switch to the Celo network."}</p><div className="mt-6 flex items-center gap-2 text-xs text-parchment-100/55"><Network size={15} aria-hidden="true" /> {isConnected ? `Chain ID ${chainId}` : "Status checking"}</div></section>
      </div>

      <section className="dashboard-reveal dashboard-reveal-delay-2 mt-8" aria-labelledby="quick-actions-heading"><div className="mb-3 flex items-center justify-between"><h2 id="quick-actions-heading" className="text-xs font-semibold uppercase tracking-[0.18em] text-parchment-100/55">Quick actions</h2><span className="text-xs text-parchment-100/40">Your command center</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Action href="/wallet#send" icon={<Send />} label="Send funds" /><Action href="/wallet#receive" icon={<Copy />} label="Receive" /><Action href="/education" icon={<BookOpen />} label="Continue learning" /><Action href="/agents" icon={<Users />} label="Find an agent" /></div></section>

      <div className="mt-6 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]"><section className="rounded-2xl border border-white/10 bg-white/[0.06] p-6"><div className="flex items-center gap-3"><ShieldCheck className="text-cyan-300" size={20} aria-hidden="true" /><h2 className="font-display font-semibold text-white">Recent activity</h2></div>{isConnected ? <p className="mt-4 text-sm leading-6 text-parchment-100/65">Your confirmed and pending transfers will appear here.</p> : <div className="mt-4 space-y-3">{demoTransactions.slice(0, 2).map((transaction) => <div key={transaction.id} className="flex items-center justify-between border-b border-white/10 pb-3 text-sm"><span className="text-parchment-100/70">{transaction.type} <span className="text-xs text-parchment-100/40">DEMO</span></span><span className="font-mono text-white">{transaction.amount} {transaction.asset}</span></div>)}</div>}<Link href="/transactions" className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-gold-300">View activity <ExternalLink size={14} aria-hidden="true" /></Link></section><section className="rounded-2xl border border-white/10 bg-white/[0.06] p-6"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-parchment-100/55">Your ecosystem</p><div className="mt-5 grid grid-cols-2 gap-4"><MiniMetric icon={<BookOpen />} value={`${demoCourses.length}`} label="Academy courses" /><MiniMetric icon={<Sprout />} value={`${demoImpact.projects}`} label="Impact projects" /></div><p className="mt-5 text-xs text-parchment-100/40">Sample discovery data · live contributions appear after connection.</p></section></div>
      </div>
    </div>
  );
}
