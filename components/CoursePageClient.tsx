...
export default function CoursePageClient({ courseId }: { courseId: string }) {
  ...
  const [done, setDone] = useState(() => {
    ...
    try { return (JSON.parse(localStorage.getItem("celoht-academy-progress") ?? "[]") as string[]).includes(courseId); } catch { return false; }
  });
  ...
  function completeLesson() {
    ...
    saved.add(courseId); localStorage.setItem("celoht-academy-progress", JSON.stringify([...saved])); setDone(true);
  }
