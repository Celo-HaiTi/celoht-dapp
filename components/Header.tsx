...
export function Header() {
  ...
  const labels: Record<string, string> = {
    Overview: t("navigation.overview"), Learn: t("navigation.learn"), Finance: t("navigation.finance"),
    Agents: t("navigation.agents"), Reforest: t("navigation.reforest"), Activity: t("navigation.activity"),
    Profile: t("navigation.profile"), Settings: t("navigation.settings"), "Trust Center": t("navigation.trustCenter"),
  };
  const renderNavItem = ({ label, href, icon: Icon }: (typeof allNav)[number]) => <li key={href}><Link href={href} aria-current={isActive(href) ? "page" : undefined} onClick={() => setMenuOpen(false)} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${isActive(href) ? "bg-gold-500/12 font-semibold text-gold-300" : "text-parchment-100/58 hover:bg-white/5 hover:text-white"}`}><Icon size={17} strokeWidth={isActive(href) ? 2.3 : 1.8} aria-hidden="true" /><span>{labels[label] ?? label}</span></Link></li>;
  ...
  return (
    ...
    <aside className="app-sidebar fixed inset-y-0 left-0 z-50 hidden w-64 flex-col border-r border-white/10 bg-navy-950 px-4 py-5 lg:flex">
      ...
      <p className="mb-2 mt-10 px-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-parchment-100/32">{t("navigation.workspace")}</p>
      <nav aria-label={t("navigation.workspace")} className="flex-1"><ul className="space-y-1">{workspaceNav.map(renderNavItem)}</ul><p className="mb-2 mt-8 px-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-parchment-100/32">{t("navigation.account")}</p><ul className="space-y-1">{secondaryNav.map(renderNavItem)}</ul></nav>
      <div className="border-t border-white/10 pt-4"><div className="flex items-center gap-2 px-3 text-xs text-parchment-100/50"><span className={`h-2 w-2 rounded-full ${isConnected && networkReady ? "bg-emerald-400" : "bg-amber-400"}`} />{isConnected && networkReady ? chain?.name : t("navigation.walletNotConnected")}</div></div>
    </aside>
    <header className="app-header sticky top-0 z-40 border-b border-white/10 bg-navy-950/90 backdrop-blur-xl lg:ml-64">
      ...
      {menuOpen && <nav id="mobile-menu" aria-label={t("navigation.mobile")} className="border-t border-white/10 bg-navy-950 px-4 pb-4 lg:hidden"><ul className="mt-3 space-y-1">{allNav.map(renderNavItem)}</ul></nav>}
    </header>
    </>
  );
}
